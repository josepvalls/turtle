...
func _process(delta):
	...
	if Game.elapsed > 2 and Game.elapsed < 2.1 and randf() < 0.1:
		pass
		#die(Vector2.ZERO)
...
func do_fire(force=false, quiet=false):
	...
	if force or fire_elapsed >= 0.0:
		#staggered_timeout += 0.3
		var projectile: Projectile
		match current_attack:
			ATTACK.SHOOT:
				projectile = projectile_template.duplicate()
			ATTACK.PULSE:
				projectile = pulse_template.duplicate()
		if not quiet:
			match current_attack:
				ATTACK.SHOOT:
					pass
					Game.play_sfx("res://sfx/Gunshot 02.wav")
				ATTACK.PULSE:
					pass
					#Game.play_sfx("res://assets/sfx/enemyPulse.wav")
		GameManager.enemy_projectiles.add_child(projectile)		
		projectile.position = $Gun.global_position
		match current_attack_aim:
			AIM.DIRECT:
				#projectile.rotation = rotation
				projectile.direction = (target.position - position).normalized()
			AIM.WAY4:
				#projectile.rotation = rotation
				projectile.rotation = stepify((target.position - position).angle(), PI*0.5)
				#projectile.direction = (target.position - position).normalized()
				projectile.direction = Vector2.RIGHT.rotated(projectile.rotation)
			AIM.DOWN:
				#projectile.rotation = Vector2.DOWN.angle()
