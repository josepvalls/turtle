...
onready var _history = preload("res://scenes/common/scenes/scenes-history.gd").new()
onready var _loader_ri = \
  preload("res://scenes/common/scenes/resource_interactive_loader.gd").new()
onready var _loader_mt = \
  preload("res://scenes/common/scenes/resource_multithread_loader.gd").new()
...
func _ready():
	...
	if ProjectSettings.get("application/run/main_scene") != cur_scene.filename:
		# Hack to call pre_start and start method to ensure compatibility with "Play Scene"
		if cur_scene.has_method("pre_start"):
			var coroutine_state = cur_scene.pre_start({})
